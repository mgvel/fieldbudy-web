...
const FormField: React.FC<EnhancedFormFieldProps> = ({
  ...
  const normalizeFile = useCallback((file: any): UploadedFile => {
    ...
    return {
      _id: file?._id ?? file?.id ?? Math.random().toString(36).slice(2, 11),
      slug: file?.slug ?? "",
      url,
    ...
    async (acceptedFiles: File[]) => {
      ...
      try {
        ...
        if (response.data?.payload) {
          ...
          const newIds = withThumbs.map((f) => f._id);
          setNewFileIds((prev) => [...prev, ...newIds]);

          // Set a timeout to remove the highlight after 5 seconds
          setTimeout(() => {
            setNewFileIds((prev) => prev.filter((id) => !newIds.includes(id)));
          }, 30000);
  ...
  const renderFilePreview = useCallback(
    (file: UploadedFile, isNew: boolean = false) => {
      return (
        <Card
          key={file._id}
          sx={{
        ...
        if (field.id === "assignment-scope") {
          return (
            ...
            >
              <Typography
                variant="h6"
                sx={{ mb: 1, color: "#333", fontSize: "16px" }}
              >
                Scope of Service
              </Typography>
                ...
                <Grid container spacing={2}>
                  {uploadedFiles.map((file, idx) => (
                    <Grid item xs={6} sm={4} md={3} key={file._id || idx}>
                      {renderFilePreview(file, newFileIds.includes(file._id))}
                    </Grid>
                  ))}
                </Grid>
              </Box>
