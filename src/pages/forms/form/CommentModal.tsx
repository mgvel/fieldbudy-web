...
const CommentModal: React.FC<CommentModalProps> = ({
  ...
  const [mentionSuggestions, setMentionSuggestions] = useState<Array<{ id: string; username: string; fullName: string; role: string }>>([]);
  ...
  const selectMention = useCallback((user: any) => {
    ...
    const newValue = newComment.substring(0, lastAtIndex + 1) + user.username + ' ';
  ...
  const renderCommentAttachment = (attachment: UploadedFile,id) => {
    ...
    return (
      <Card sx={{ mt: 1, maxWidth: 300, borderRadius: 2 }}>
        <CardContent sx={{ p: 1.5, '&:last-child': { pb: 1.5,background:"#F5F5F5" } }}>
          <Box sx={{ display: 'flex',flexDirection:"column", alignItems: 'end', gap: 1,backfaceVisibility:"hidden" }}>
            {isImage ? (
             <img
             src={`${API_BASE_URL}/comments/${id}/files/${encodeURIComponent(attachment.filename)}`}
           ...
           />
            ...
            <Box>
              {/* <IconButton size="small" onClick={() => window.open(`${API_BASE_URL}/comments/${id}/files/${encodeURIComponent(attachment.filename)}`, '_blank')}>
  ...
  const renderComment = (comment: Comment) => {
    ...
    const commentUser = availableUsers.find(user => user._id === comment.author);
    return (
      <Box
        key={comment._id}
        sx={{
      ...
      >
        <Box sx={{ 
          ...
          <Badge
            ...
            badgeContent={
              <Box sx={{
                ...
                height: 12,
                borderRadius: '50%',
                bgcolor: commentUser?.role === 'FE' ? '#4CAF50' : 
                         commentUser?.role === 'TW' ? '#2196F3' : 
                         commentUser?.role === 'QR' ? '#FF9800' : '#9C27B0',
          ...
          <Box sx={{ maxWidth: 'calc(100% - 60px)' }}>
            ...
            >
              <CardContent sx={{ p: 2, '&:last-child': { pb: 2 }, minWidth:"230px" }}>
                <Box sx={{ 
                  display: 'flex', 
                  ...
                  <Typography variant="caption" sx={{ color: isOwnComment ? 'rgba(255,255,255,0.7)' : 'text.secondary' }}>
                    ...
                    if (part.startsWith('@')) {
                      ...
                      return (
                        <Chip
                          ...
                          size="small"
                          sx={{
                            backgroundColor: isOwnComment ? 'rgba(255,255,255,0.2)' : '#E3F2FD',
                ...
                {comment.attachments?.map((attachment) => renderCommentAttachment(attachment,comment._id))}
              </CardContent>
            </Card>
          </Box>
        </Box>
        ...
        >
          {isLoading ? (
            ...
            <Box sx={{ 
              ...
              />
              <Typography variant="h6" sx={{ mb: 1 }}>No comments yet</Typography>
              <Typography variant="body2">Start the conversation by sending a message</Typography>
            </Box>
          ) : (
            ...
            {mentionSuggestions.map((user, index) => (
              <Box
                ...
                sx={{
                  p: 1.5,
                  cursor: 'pointer',
                  backgroundColor: index === activeMentionIndex ? '#f5f5f5' : 'white',
              ...
              >
                <Avatar 
                  sx={{ 
                    width: 32, 
                    height: 32,
                    bgcolor: user.role === 'FE' ? '#4CAF50' : 
                             user.role === 'TW' ? '#2196F3' : 
                             user.role === 'QR' ? '#FF9800' : '#9C27B0'
                  }}
                >
