...
const MyPendingSiteVisits = () => {
  ...
  const getSiteVisits = async () => {
    try {
      ...
      const filterVisted = data.payload.filter((item:any)=>item.isVisited===false)
      setSiteVisits(filterVisted as SiteVisit[]);
    } catch (err) {
  ...
  return (
    <div className="w-[75vw] mx-auto">
      ...
      <header className="flex items-center justify-between">
        <h2 className="text-xl font-semibold">My Pending Site Visit</h2>
      </header>
        <DynamicTable
          columns={columns}
          data={tableData}
...
export default MyPendingSiteVisits;
